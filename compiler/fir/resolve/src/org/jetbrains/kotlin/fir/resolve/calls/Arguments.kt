...
private fun argumentTypeWithSuspendConversion(
    ...
    // Or is a subtype of functional type
    if (argumentType is ConeClassLikeType &&
        argumentType.isSubtypeOfFunctionalType(session, expectedType.typeArguments.size)
    ) {
        return argumentType.findInvokeSymbol(session, scopeSession, expectedType.typeArguments.size)?.let { invokeSymbol ->
